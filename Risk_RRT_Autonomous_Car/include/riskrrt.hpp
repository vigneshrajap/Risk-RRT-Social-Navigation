...
#include <icars_2d_map_manager/Status.h>
#include <Eigen/Dense>
...
struct Params{
  ...
  double maxSteeringAngle; //maximum steering angle (rad/s)
};
...
public:
  ...
  custom_pose global_goal;//final goal set by user (=/= random goal)
  custom_pose local_goal[100];//local goal set by user (=/= random goal)
  custom_pose final_goal;
  // custom_pose sub_goal;//sub goal set by user (=/= random goal)
 ...
 ros::Subscriber lookaheadSubscriber;
  ...
  void lookaheadCallback(const std_msgs::Int8 msg);
  void initlookaheadSub();
