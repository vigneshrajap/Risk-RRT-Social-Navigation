...
void errortrajCallback(geometry_msgs::Pose msg1);
...
void registerDataOnFile1(geometry_msgs::Pose error_traj);
...
void registerDataOnFile1(geometry_msgs::Pose error_traj)
{
       error1_t.position.x = error_traj.position.x;
       error1_t.position.y = error_traj.position.y;
 
std::ofstream resultOutput1;

    resultOutput1.open("/home/user/catkin_ws/src/vignesh/riskrrt/error_traj.txt", std::ofstream::out | std::ofstream::app);
	    if (resultOutput1.fail())
           ...
           else
    		{
			  ...
			  resultOutput1 << error1_t.position.x << ";";
			  resultOutput1 << error1_t.position.y << ";";
		    }
  	resultOutput1 << endl;
	resultOutput1.close();
...
{
	 traj1.position.x=traj.points[i].x;
	 traj1.position.y=traj.points[i].y;
    ...
    //cout << " Registering:";.c_str()
    resultOutput3.open("/home/user/catkin_ws/src/vignesh/riskrrt/trajectory.txt", std::ofstream::out | std::ofstream::app);
	    if (resultOutput3.fail())
			...
			resultOutput3 << traj1.position.x << ";";
			resultOutput3 << traj1.position.y << ";";
			//resultOutput << dat.pose.position.z<< ";";
...
int main (int argc, char** argv)
{
  ...
  //Connect to ROS
  ros::init(argc, argv, "Data_registerer_node");
 ...
 resultOutput1.open("/home/user/catkin_ws/src/vignesh/riskrrt/error_traj.txt", std::ofstream::out | std::ofstream::trunc);

      resultOutput1 << " error_traj x;";
      resultOutput1 << " error_traj y;";
      resultOutput1 << endl;
 ...
 resultOutput3.open("/home/user/catkin_ws/src/vignesh/riskrrt/trajectory.txt", std::ofstream::out | std::ofstream::trunc);
